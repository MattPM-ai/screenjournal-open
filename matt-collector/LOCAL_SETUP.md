...
This guide will help you run the matt-collector service locally for testing.
...
```bash
cd /Users/alexanderwestlake/repos/matt-collector
go mod download
```
...
INFLUXDB_DATABASE=matt-metrics-dev
...
**Note:** The `/mock-auth` endpoint now requires authentication with a valid JWT token from mattpm-backend. You must first obtain a token from mattpm-backend, then use it to access this endpoint.

```bash
# First, obtain a backend JWT token from mattpm-backend
...
**Important:** The `JWT_SECRET` environment variable must match the secret used by mattpm-backend for token validation to work.
...
docker run -d \
  --name influxdb \
  -p 8181:8181 \
  -e INFLUXDB_DATABASE=matt-metrics-dev \
