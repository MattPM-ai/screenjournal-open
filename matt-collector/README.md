...
- Local development: `http://matt-collector.localhost`
- Production: Configure `DOMAIN` in `.env` (e.g., `matt-collector.yourdomain.com`)
...
# View logs
docker-compose logs -f matt-collector
...
| `JWT_SECRET` | Secret key for signing JWT tokens (must match mattpm-backend JWT_SECRET for backend token validation) | `your-secret-key-change-in-production` |
...
| `DOMAIN` | Domain for Caddy reverse proxy (Docker only) | `matt-collector.localhost` |
...
Generates a JWT token for the authenticated user and organization. This endpoint is protected and requires a valid JWT token from mattpm-backend (shared JWT_SECRET).

**Authentication:**
- Requires a valid JWT token from mattpm-backend in the `Authorization` header
- The token must be signed with the same `JWT_SECRET` used by mattpm-backend
...
- Change `JWT_SECRET` to a strong, random value (must match mattpm-backend JWT_SECRET)
