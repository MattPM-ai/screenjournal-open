...
Comprehensive test dataset for the Matt-Collector employee monitoring system, containing realistic activity data for 8 users across 9 days (November 15-23, 2025).
...
**Automated upload:**
```bash
./upload_with_curl.sh matt-metrics-dev "your-token-here" "http://195.74.52.54:8181"
```
...
grep -v '^#' influxdb/app_usage.lp | grep -v '^$' | \
  curl -X POST "http://localhost:8181/api/v3/write_lp?db=matt-metrics-dev" \
