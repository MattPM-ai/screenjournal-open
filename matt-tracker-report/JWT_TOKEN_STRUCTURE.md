...
This document describes the structure of JWT tokens generated by `mattpm-backend` and validated by this service.
...
## Token Generation (mattpm-backend)

Tokens are generated in `mattpm-backend/src/users/utils/jwt.ts`:
...
## Token Validation (matt-tracker-report)
...
1. **JWT_SECRET**: Must match between `mattpm-backend` and `matt-tracker-report`
