...
pub async fn obtain_token(
    ...
    if let Some(token) = app_jwt_token {
        ...
        log::warn!("No app JWT token provided to obtain_token()");
    }
    ...
    // Add Authorization header if app JWT token is provided
    if let Some(token) = app_jwt_token {
        ...
        log::warn!("[AUTH] No app JWT token provided, skipping Authorization header");
    }
