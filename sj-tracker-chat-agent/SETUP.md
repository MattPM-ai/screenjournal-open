...
1. **Go Backend Running**: The `sj-tracker-report` service must be running on port 8085
...
1. **Start the Go backend** (in `sj-tracker-report`):
```bash
cd sj-tracker-report
go run cmd/server/main.go
```

2. **Start the Python agent** (in `sj-tracker-chat-agent`):
```bash
python main.py
```
...
1. Add tool definition in `sj-tracker-report/internal/services/chat_tools.go`
