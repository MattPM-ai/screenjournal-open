 ...
 * ============================================================================
 * 
 * PURPOSE: Displays formatted report data
 * 
 * DESCRIPTION:
 * This component renders the formatted report HTML using the formatting
 * utilities. It handles both successful reports and error states.
 * 
 * DEPENDENCIES:
 * - /lib/reportFormatter.ts: Formatting utility functions
 * 
 * ============================================================================
 */
...
/**
 * Parses the report data, handling various response formats
 * 
 * INPUTS:
 ...
 */
function parseReportData(data: any): any {
  try {
    if (!data) {
      console.warn('parseReportData: data is null or undefined')
      return null
    }
    ...
    if (data.output && typeof data.output === 'string') {
      try {
        ...
        console.error('parseReportData: Failed to parse output string:', parseError)
        try {
          ...
          console.error('parseReportData: Failed to parse as double-encoded JSON:', doubleParseError)
        }
      }
    }
    ...
    if (typeof data === 'string') {
      try {
        ...
        if (parsed && parsed.output && typeof parsed.output === 'string') {
          return parseReportData(parsed)
        }
      } catch (stringParseError) {
        console.error('parseReportData: Failed to parse string data:', stringParseError)
      }
    }
    
    console.warn('parseReportData: Could not find valid report structure in data:', data)
    return null
  } catch (error) {
    console.error('parseReportData: Unexpected error:', error, data)
    return null
  }
}

export default function ReportDisplay({ reportData, onNewReport }: ReportDisplayProps) {
  ...
  const handleDownloadPDF = () => {
    if (!parsedData) {
      console.error('No valid report data available for PDF generation')
      return
    }
    ...
    try {
      ...
      if (parsedData.organizations && parsedData.organizations.length > 0) {
        ...
        filename = `${sanitizedOrgName}-report`
      }

      // Add date if available from first user's overall report
      if (parsedData.organizations?.[0]?.users?.[0]?.overallReport) {
        const overallReport = parsedData.organizations[0].users[0].overallReport
        if (overallReport.periodStart && overallReport.periodEnd) {
          const startDate = new Date(overallReport.periodStart).toISOString().split('T')[0]
          const endDate = new Date(overallReport.periodEnd).toISOString().split('T')[0]
      ...
      generatePDF(parsedData, filename)
    } catch (error) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsedData) {
        ...
        reportOutputRef.current.innerHTML = `
          <div class="error-message">
            <strong>Unable to format report.</strong> The response structure may be different than expected. 
      ...
      if (reportOutputRef.current) {
        reportOutputRef.current.innerHTML = `
          <div class="error-message">
            <strong>Error displaying report:</strong> ${error instanceof Error ? error.message : 'Unknown error'}
          </div>
          <pre class="json-output">${JSON.stringify(reportData, null, 2)}</pre>`
      }
    }
  ...
  return (
    <div className="w-full max-w-6xl bg-white rounded-lg shadow-sm border border-gray-200 p-10">
      <h1 className="text-3xl font-semibold text-gray-900 mb-8">Report Results</h1>
