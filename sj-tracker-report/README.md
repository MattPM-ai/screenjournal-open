# ScreenJournal Tracker Report Backend
...
All API endpoints (except `/health`) require JWT authentication. The service validates JWT tokens generated by `screenjournal-backend` but does not generate tokens itself.
...
Set `JWT_SECRET` in your `.env` file to match the secret used by `screenjournal-backend`:
...
- `JWT_SECRET`: JWT secret key (must match `screenjournal-backend` JWT_SECRET)
